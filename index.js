...
function renderWord(entry) 
{
  ...
  // Build the save button label
    let saveLabel = " Add Favorite";
    let saveClass = "icon-btn save-btn";
    if (saved) 
        {
            saveLabel =  "Add Favorite";
...
{
  const raw = localStorage.getItem(FAVORITES_KEY);  //Favorite local Storage
...
function handleSaveToggle() 
{
    ...
    renderWord(currentEntry);  // Re-draw the word so the save button updates too
};
...
function displayFavorites() 
{
  ...
  favoritesListEl.innerHTML = html;  //InnerHtml
...
function toggleTheme() 
{
    ...
    localStorage.setItem(THEME_KEY, isDark ? "dark" : "light");   //Theme local storage
};
